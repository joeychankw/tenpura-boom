...
function addHighScore(record, node) {
    ...
    name.setAttribute("dy", 35); // dy: y distance from last <test>?
    ...
    // score.setAttribute("dy", 0); // dy == 0 default?
...
function setCookie(name, value, expires, path, domain, secure) {
    var curCookie = name + "=" + escape(value) + // encode string, '~' --> "%7E"
        ((expires) ? "; expires=" + expires.toGMTString() : "") + // Date object --> string
        ((path) ? "; path=" + path : "") +
        ((domain) ? "; domain=" + domain : "") +
        ((secure) ? "; secure" : "");
    // console.log(curCookie);//
    document.cookie = curCookie;
    // document.cookie = "value=xyz";//
...
function getCookie(name) {
    ...
    return unescape(dc.substring(begin + prefix.length, end)); //decode string, "%7E" --> '~'
}
